namespace MusysIR {
    public void number_module(Module module)
    {
        foreach (var entry in module.global_def) {
            var gobj = entry.value;
            if (gobj is Function)
                number_function(static_cast<Function>(gobj));
        }
    }
    ...
    public int number_function(Function fn, NumberValueFlags flags = 0)
    {
        int number = 0;
    ...
    private int _number_basic_block(BasicBlock block, int begin, NumberValueFlags flags)
    {
    ...
    private bool _instruction_should_number(Instruction inst, NumberValueFlags flags)
    {
